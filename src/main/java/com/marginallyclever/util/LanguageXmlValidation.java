...
public class LanguageXmlValidation {
    ...
    public static void outputlLocalInfo(Locale l) {

	System.out.printf("locale : language=%s (%s) contry=%s (%s) // %s %s // %s ?~= %s\n",
		l.getLanguage(), l.getISO3Language(), l.getCountry(), l.getISO3Country(),
		l.getDisplayLanguage(), l.getDisplayCountry(), l.getDisplayName(), l.getDisplayName(l));
    }

    public static void outputlLocalsInfo(Locale[] locales) {
	if (locales == null) {
	    return;
	}
	System.out.printf("locales.lenght=%d\n", locales.length);
